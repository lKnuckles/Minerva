    ...
    class MapLoader
    {
        ...
        public Dictionary<int, Map> LoadMaps()
        {
            ...
            foreach (var n in node.Elements("map"))
            {
                ...
                var mobs = LoadMobs(id);
                var terrain = LoadTerrain(threadmap);

                var map = new Map(id, mobs, warps, terrain, (deadReturn.ContainsKey(mI) ? deadReturn[mI][0] : 1), (deadReturn.ContainsKey(mI) ? deadReturn[mI][1] : 1), (warpNPCs.ContainsKey(mI) ? warpNPCs[mI] : null));
        ...
        List<MobEntity> LoadMobs(int map)
        {
            var xml = XDocument.Load("data/MobList.xml");
            var node = xml.Root;
            ...
            foreach (var n in node.Elements("World"))
            {
                ...
                foreach (var _n in n.Element("Mobs").Elements("mob"))
                {
                    ...
                    mobs.Add(new MobEntity(mI, Id, PosX, PosY, PosX, PosY, 10, 100, 100));
                    mI++;
                }
            }
